...
class setting_config:
    ...
    semi_supervised = False  # 默认全监督模式，设为True启用Mean-Teacher半监督
    lambda_consistency = 0.05  # 一致性损失权重
    ema_alpha = 0.99  # EMA滑动平均系数
    test_weights = './pretrained_weights/best.pth'
    resume_model = './pretrained_weights/best.pth'

    datasets = 'ISIC2017'
    if datasets == 'ISIC2017':
        data_path = './dataprepare/'
    ...
    # ================== 训练模式配置 ==================
    # 只保留标准监督训练相关配置项，删除所有半监督、对比学习、交叉监督相关内容。
    ...
    # 损失函数配置
    criterion = 'BceDiceLoss'  # 可选: 'BceDiceLoss', 'DiceLoss', 'BCELoss', 'CrossEntropyLoss'
